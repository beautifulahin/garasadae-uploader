...
# ★**launchd 는 PATH 를 안 물려준다** (2026-08-23, H-209).
#   저 혼자 도는 판은 `/usr/bin:/bin:/usr/sbin:/sbin` 넷뿐이라 홈브루로 깐 연장이
#   하나도 안 보인다. 그래서 점검이 `deno: command not found` 로 엎어졌고,
#   **열 시간 동안 열다섯 분마다 같은 자리에서 죽었다** — 그 사이 개인판은
#   옛 판 그대로 돌았다. 터미널에서 손으로 돌리면 잘 되니 더 안 보였다.
...
# ★**같은 자리에서 되풀이해 죽으면 점점 크게 외친다** (2026-08-23, H-209).
#   여태는 실패할 때마다 똑같은 쪽지 한 장이었다. 열다섯 분마다 오는 똑같은 글은
#   사람이 곧 안 읽는다 — 열 시간을 죽어 있어도 "늘 오던 그거" 로 묻혔다.
#   몇 바퀴째인지를 세어 **몇 시간째 멈춰 있는지**를 글에 적는다.
연이어실패() {
  local n=$(( $(cat ~/.volcano/개인판연속실패 2>/dev/null || echo 0) + 1 ))
  echo "$n" > ~/.volcano/개인판연속실패
  echo "$n"
}
trap 'rc=$?; if [ $rc -ne 0 ]; then n=$(연이어실패); 텔레그램 "⚠️ 업로더 개인판 갱신이 ${rc} 로 멈췄다 — 이번이 ${n}번째다(약 $((n*15))분째 옛 판이 돈다). 터미널에서 개인판.sh 를 돌려 무엇이 막혔는지 보라."; fi' EXIT
...
# ★연장이 없어서 죽은 것과 코드가 틀려서 죽은 것은 **다른 고장**이다.
#   한 덩이로 "점검 실패" 라고만 하면 소스를 뒤지다 하루를 버린다 — 먼저 갈라 본다.
for tool in deno node; do
  command -v "$tool" >/dev/null || {
    로그 "연장이 없다: $tool — 점검을 돌리지도 못한다"
    텔레그램 "⚠️ 업로더 개인판: $tool 을(를) 못 찾는다. 코드가 아니라 **연장 자리(PATH)** 탈이다. 저 혼자 도는 판은 홈브루 자리를 물려받지 못한다."
    exit 1
  }
done
CHKOUT=$(./점검.sh 2>&1) || {
  로그 "점검 실패 — 갈아끼우지 않는다"
  텔레그램 "⚠️ 업로더 개인판 점검 실패 — 옛 판을 그대로 둔다$(echo "$CHKOUT" | grep '❌' | head -3 | sed 's/^/\n/')"
  exit 1
}
...
JSON
rm -f ~/.volcano/개인판연속실패      # 살아났으니 셈을 지운다
